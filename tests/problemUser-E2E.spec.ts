...
test.describe('Testeo del proceso de compras End To End', () => {
    ...
    test('2. Seleccion de multiples items y agregado al carrito', async ({ page }) => {
        test.fail(true, 'Problem_user no selecciona correctamente los items elegidos');
        await expect(page.locator('.inventory_item').first()).toBeVisible();
    ...
    test('3. Flujo completo de compra con errores con logueo de usuario Problem_user', async ({ page }) => {
        test.fail(true, 'Flujo de compras con Problem_user sin camino feliz.');
